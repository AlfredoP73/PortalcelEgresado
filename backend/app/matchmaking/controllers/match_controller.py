...
@router.post("/recalcular", response_model=list[schemas.MatchOut])
def recalcular(payload: schemas.RecalcularRequest, db: Session = Depends(get_db)):
    """
    Trigger interno llamado por los microservicios `companies` (al crear/editar
    una vacante) y `graduates` (al actualizar el perfil/hoja de vida).
    """
...
@router.get("/graduate/{graduate_id}", response_model=list[schemas.MatchOut])
...
@router.get("/vacancy/{job_offer_id}", response_model=list[schemas.MatchOut])
...
@router.get("/graduate/{graduate_id}/vacancy/{job_offer_id}", response_model=schemas.MatchOut)
